        ...
        if (!this.gridInitialized) {
            ...
            for (let row = -1; row < this.gridSize; row++) {
                for (let col = -1; col <= this.gridSize; col++) {
                    ...
                    // התיקון לקווי המתאר: אובייקט חדש שיצייר קווים עבים במיוחד מעל התמונה!
                    let strokeGfx = this.add.graphics();
                    strokeGfx.setDepth(baseRowDepth + 1.5);
                    strokeGfx.lineStyle(5, 0x000000, 1); // עובי 5 בולט!
                    ...
                    if (height > baseHeight && tileType !== 2) {
                        if ((tileType === 1 || isExternalWall) && this.textures.exists('tex_wall')) {
                            ...
                            // התיקון לשטוחות: הוספת ה-col למזהה התמונה! (c${col})
                        ...
                        // מצייר את המסגרת העבה של הצד מעל הכל
                        strokeGfx.beginPath();
                    ...
                    // מצייר את המסגרת העבה העליונה מעל הכל
                    strokeGfx.beginPath();
        ...
        this.spikeTraps.forEach(trap => {
            ...
            if (this.trapObjects && this.trapObjects[key]) {
                ...
                if (trap.active) {
                    ...
                    // התיקון לקווים של הקוצים: עובי 3 כדי שיראו אותם ברור!
        ...
        fetch('/api/win', {
            ...
            fetch('/api/announce', {
                ...
                body: JSON.stringify({
                    ...
                    tries: this.maxTries - window.PlayerData.triesLeft, // חישוב כמות הנסיונות
                    crystals: this.collectedCrystals,
                    moves: actionCost,
                    biome: this.currentBiome,
                    themeColor: '#4CAF50'
