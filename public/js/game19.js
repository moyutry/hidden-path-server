...
class GameScene extends Phaser.Scene {
        ...
        this.input.on('pointerdown', (pointer) => {
            ...
            if (clickedCol !== -1) {
                ...
                if (tileType === 9) { title = 'Exit Portal'; subtitle = 'Level Goal'; }
                    ...
                    subtitle = isMovingBridge ? `Moves in ${this.bridgeLogic.pauseTimer} turn(s) (Every 2)` : 'Safe'; 
                }
                else if (tileType === 2) { title = 'Deep Water'; subtitle = 'Danger: Sinks Player'; }
                else if (tileType === 3) { 
                    title = 'Spike Trap'; 
                    subtitle = `Changes in ${this.trapTimer} turn(s) (Every 2)`; 
                }
    ...
    showGameOverScreen() {
        ...
        fetch('/api/announce', {
                ...
                body: JSON.stringify({
                    ...
                    crystals: this.collectedCrystals, moves: actionCost, 
        ...
        this.add.text(this.scale.width/2, 450, '💀\nGAME OVER', { fontFamily: this.comicFont, fontSize: '90px', fill: '#FF5252', fontStyle: 'bold', stroke: '#000', strokeThickness: 12, align: 'center' }).setOrigin(0.5).setDepth(10001);
    ...
    showWinScreen() {
        ...
        fetch('/api/win', {
            ...
            fetch('/api/announce', {
                ...
                body: JSON.stringify({
                    channelId: window.PlayerData.channelId, username: window.PlayerData.username, avatarUrl: window.PlayerData.avatarUrl,
                    isWin: true, score: data.totalScore, tries: triesUsed,
            ...
            // ... (מכאן מתחיל הציור של הפאנל עם הקוד שכבר יש לך. רק זכור לשנות גם פה את שורת הטקסט של הבלוקים כך:)
            // this.add.text(this.scale.width/2, 680, `🧩 Blocks Used: ${finalActionCost} (Par: ${data.parScore})`, ...
            ...
            this.add.text(this.scale.width/2, 680, `🧩 Blocks Used: ${actionCost} (Par: ${data.parScore})`, { fontFamily: this.comicFont, fontSize: '45px', fill: '#FFFFFF' }).setOrigin(0.5).setDepth(10001);
    ...
    executeNextAction() {
        ...
        if (this.currentStep >= this.actionQueue.length) { 
            ...
            if (!onExit) {
                this.cumulativeActions += this.calculateCurrentActionCost(); // מוסיף מהלכים לפעם הבאה
