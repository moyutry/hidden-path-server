...
class LobbyScene extends Phaser.Scene {
    ...
    create() {
        ...
        if (isSolved) {
            ...
            this.createButton(this.scale.width/2, 850, '🔒 0 LIVES', 0xFF5252, () => {});
        } else {
...
class GameScene extends Phaser.Scene {
    ...
    updatePlayerVisualPosition(duration = 0) {
        ...
        if (this.playerSprite && this.playerSprite.active) {
            ...
            if (skinConfig) this.playerSprite.setTexture(`${skinConfig.id}_${this.playerLogic.direction}`);
    ...
    showWinScreen() {
        this.isPlaying = true; 
        let finalActionCost = this.cumulativeActions + this.calculateCurrentActionCost();
        let triesUsed = this.maxTries - window.PlayerData.triesLeft;
        
        fetch('/api/win', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': window.PlayerData.accessToken },
            body: JSON.stringify({ actionCost: finalActionCost, triesUsed: triesUsed })
            ...
            fetch('/api/announce', {
                ...
                headers: { 'Content-Type': 'application/json', 'Authorization': window.PlayerData.accessToken },
                body: JSON.stringify({
                    channelId: window.PlayerData.channelId, isWin: true, score: data.totalScore, tries: triesUsed,
                    crystals: this.collectedCrystals, moves: finalActionCost, biome: this.currentBiome, 
                    themeColor: '#' + (window.OwnedAssets.bgs.find(b => b.id === window.PlayerData.currentBG) || {uiMain: 0x4CAF50}).uiMain.toString(16).padStart(6, '0'),
                    skin: window.OwnedAssets.skins.find(s => s.id === window.PlayerData.currentSkin),
                    bgTheme: window.OwnedAssets.bgs.find(b => b.id === window.PlayerData.currentBG),
                    pack: window.OwnedAssets.packs.find(p => p.id === window.PlayerData.currentPack)
                })
    ...
    executeNextAction() {
        ...
        if (this.currentStep >= this.actionQueue.length) { 
            ...
            if (!onExit) {
                this.cumulativeActions += this.calculateCurrentActionCost();
