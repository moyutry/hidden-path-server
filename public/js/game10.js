...
//             // חליפת המגן: מפעיל את האנימציה אך ורק אם היא קיימת, תקינה, ויש לה פריימים!
...
//                     try { waterSprite.play('water_flow', true); } catch(e) { console.log("Water anim skipped"); }
//                 }
//             }
...
//                             if (!isDynamic) { rowGroup.add(sideImg); rowGroup.add(sideMaskGfx); }
//                         } else {
...
//                         if (!isDynamic) { rowGroup.add(topImg); rowGroup.add(maskGfx); }
//                     } else {
...
//                     } else {
//                         localGfx.setDepth(baseRowDepth + 1);
//                         if (topImg) topImg.setDepth(baseRowDepth + 2);
//                         let spikeGfx = this.add.graphics(); spikeGfx.setDepth(baseRowDepth + 3);
//                         this.trapObjects[`${col}_${row}`] = { baseGfx: localGfx, img: topImg, maskGfx: maskGfx, spikeGfx: spikeGfx };
//                     }
//                 }
                
//                 if (rowGroup.getChildren().length > 0) {
...
//             // --- 5. הקריסטלים ---
//             if (this.crystalsLogic) {
//                 this.crystalsLogic.forEach(c => {
//                     if (this.textures.exists('tex_crystal')) {
//                         const pos = this.getPerspective(c.x, c.y, 10);
//                         let cImg = this.add.image(pos.x, pos.y, 'tex_crystal');
//                         cImg.setDisplaySize(35 * pos.scale, 45 * pos.scale);
...
//                             ease: 'Sine.easeInOut'
//                         });
//                         this.crystalObjects.push({ logic: c, img: cImg });
//                     }
//                 });
//             }
//         }
...
//     // פונקציה חדשה לעדכון הקריסטלים!
//     updateCrystalsVisuals() {
//         if (!this.crystalObjects) return;
//         this.crystalObjects.forEach(cObj => {
//             if (cObj.logic.collected) {
//                 cObj.img.setVisible(false); // מעלים אם נאסף
//             } else {
//                 cObj.img.setVisible(true);
//                 let depth = (cObj.logic.y + 2) * 30 + 26; // מוודא שהם מעל המפה האפויה
//                 cObj.img.setDepth(depth);
//             }
//         });
//     }

//     updatePlayerVisualPosition(duration = 0) {
//         // התיקון לארור ה-duration! מוודא שהשחקן קיים לפני שמנסים להזיז אותו
...
//             } else {
//                 let targetX = this.bridgeLogic.x;
//                 let targetY = this.bridgeLogic.y;
...
class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        // הגנה הרמטית מארור ה-Duration של אנימציית המים!
    ...
    // פונקציית ה"פוטושופ" האוטומטית! חותכת את התמונות בזיכרון במקום מסכות כבדות
    createMaskedTexture(textureKey, pts, cutKey) {
        ...
        ctx.closePath();
        ctx.clip(); // גוזר בדיוק לצורת היהלום

        ctx.drawImage(srcImg, 0, 0, w, h);
        this.textures.addImage(cutKey, canvas); // שומר תמונה נקייה בזיכרון
        return cutKey;
    }
    ...
    drawRealisticGrid() { 
        if (!this.gridInitialized) {
            ...
            // --- 3. ציור הרצפות והקירות 1 ל-1 כמו פעם, אבל סופר-מהיר! ---
            for (let row = -1; row < this.gridSize; row++) {
                for (let col = -1; col <= this.gridSize; col++) {
                    let isExternalWall = (row === -1 || col === -1 || col === this.gridSize);
                    let tileType = 1; 
                    ...
                    // ציור בסיס תלת מימד שחור ולבן
                    if (height > baseHeight && tileType !== 2) {
                        if ((tileType === 1 || isExternalWall) && this.textures.exists('tex_wall')) {
                            let sidePts = [bl, br, base_br, base_bl];
                            let sideCutKey = `tex_wall_side_r${row}_h${height}_b${baseHeight}`;
                    ...
                    if (textureKey && this.textures.exists(textureKey)) {
                        let pts = [tl, tr, br, bl];
                        let cutKey = `${textureKey}_top_r${row}_h${height}`;
                        ...
                        if (finalKey) {
                            ...
                            topImg.setDepth(baseRowDepth + 1); // בלי מסכה ובלי Scale, הקובץ כבר מושלם!
                        }
                    } else {
                        localGfx.fillStyle(Phaser.Display.Color.HexStringToColor(topColor).color, 1); localGfx.beginPath();
                        localGfx.moveTo(tl.x, tl.y); localGfx.lineTo(tr.x, tr.y); localGfx.lineTo(br.x, br.y); localGfx.lineTo(bl.x, bl.y);
            ...
            if (this.crystalsLogic) {
                this.crystalsLogic.forEach(c => {
                    if (this.textures.exists('tex_crystal')) {
                        ...
                        this.tweens.add({
                            targets: cImg, y: pos.y - (15 * pos.scale), duration: 1000, yoyo: true, repeat: -1, ease: 'Sine.easeInOut'
                        });
                        this.crystalObjects.push({ logic: c, img: cImg });
                    }
                });
        ...
        if (this.playerContainer && this.playerContainer.active && this.bridgeVisualY !== undefined) {
            let pRow = this.playerLogic.y; 
            let playerDepth = (pRow + 2) * 30 + (this.playerLogic.x + 2) * 2 + 5; 
            if (this.bridgeLogic && pRow === this.bridgeLogic.y && this.playerLogic.x === this.bridgeLogic.x) {
    ...
    updateTrapsVisuals() {
        this.spikeTraps.forEach(trap => {
            ...
            if (this.trapObjects && this.trapObjects[key]) {
                ...
                // מייצר או שולף את תמונת המלכודת החתוכה
                ...
                let pts = [tl, tr, br, bl];
                let cutKey = `${textureKey}_top_r${trap.y}_h0`;
                let finalKey = this.createMaskedTexture(textureKey, pts, cutKey);
    ...
    createCrystals() {
        this.crystalsLogic.forEach(c => {
            ...
            // תוקן העומק כך שירחף מושלם מעל המפה האפויה!
            c.sprite.setDepth((c.y + 2) * 30 + 26);
    ...
    checkCollisions() {
        // הגנה שמוודאת שהשחקן לא יצא מגבולות המערך
            ...
            if (this.bridgeLogic.pauseTimer > 0) {
                ...
                this.tweens.add({
                    ...
                    onUpdate: () => {
                        this.updateBridgeVisuals();
                        if (playerOnBridge && this.playerContainer) {
                            // שימוש בנוסחת העומק החדשה בזמן שהגשר זז!
                            let pDepth = (Math.ceil(this.bridgeVisualY) + 2) * 30 + 28;
                            this.playerContainer.setDepth(pDepth);
                        }
                    }
                });
    ...
    executeNextAction() {
        ...
        if (this.currentStep >= this.actionQueue.length) { 
            if (this.mapData[this.playerLogic.y] && this.mapData[this.playerLogic.y][this.playerLogic.x] !== 9) {
                this.triggerDeath('out_of_moves');
            } else if (this.mapData[this.playerLogic.y] && this.mapData[this.playerLogic.y][this.playerLogic.x] === 9) {
                this.isPlaying = false; 
                this.renderVisualQueue(); 
            }
            return; 
