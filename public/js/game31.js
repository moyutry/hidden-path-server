...
//         if(window.DailyData.skins) window.DailyData.skins.forEach(i => previewItems.add(i.id));
...
// ==========================================
// BOOT SCENE - MASSIVE 3D CINEMATIC INTRO
// ==========================================
class BootScene extends Phaser.Scene {
    constructor() { super('BootScene'); }
    ...
    async create() {
        // --- 1. אווירת חלל התחלתית (מסווה את הטעינה) ---
        this.cameras.main.setBackgroundColor('#030305');
        
        let starGfx = this.make.graphics({x:0, y:0, add:false});
        starGfx.fillStyle(0xffffff, 1); starGfx.fillCircle(2, 2, 2); starGfx.generateTexture('flare', 4, 4);

        this.ambientStars = this.add.particles(0, -50, 'flare', {
            x: { min: 0, max: this.scale.width },
            y: { min: 0, max: this.scale.height },
            lifespan: 3000,
            speedY: { min: 100, max: 300 }, // כוכבים זורמים לאט כלפי מטה
            scale: { start: 0, end: 1 },
            alpha: { start: 0, end: 0.5 },
            blendMode: 'ADD'
        });

        // טקסט טעינה חריג (מוסתר כברירת מחדל)
        this.fallbackLoading = this.add.text(this.scale.width/2, this.scale.height - 50, 'Loading internal systems...', { 
            fontFamily: '"Arial Black", sans-serif', fontSize: '20px', fill: '#666666' 
        }).setOrigin(0.5).setAlpha(0);

        // --- 2. לוגיקת הטעינה החכמה ---
        let isLoaded = false;
        let isCinematicTimePassed = false;

        // נותנים לסינמטיקה מינימום 2.5 שניות לרוץ בשביל האווירה
        this.time.delayedCall(2500, () => {
            isCinematicTimePassed = true;
            if (isLoaded) {
                this.triggerMarvelClimax();
            } else {
                this.fallbackLoading.setAlpha(1); // מציג את הטקסט רק אם הטעינה מתעכבת!
            }
        });

        // ==========================================
        // משיכת הנתונים מהשרת ברקע
        // ==========================================
        try {
            const { DiscordSDK } = await import('/discord-sdk.js');
            const discordSdk = new DiscordSDK('1518734375934754816');
    ...
    triggerMarvelClimax() {
        this.fallbackLoading.destroy();
        this.ambientStars.destroy(); // מעלימים את כוכבי הרקע

        // פלאש לבן שמתחיל את הקליימקס
        this.cameras.main.flash(300, 255, 255, 255);

        // --- יצירת הטקסט התלת-מימדי (Extrusion) ---
        this.titleContainer = this.add.container(this.scale.width / 2, this.scale.height / 2);
        
        const depth = 16; // עומק התלת מימד
        for (let i = depth; i >= 0; i--) {
            let isTop = (i === 0);
            // השכבות התחתונות שחורות/אפורות כדי ליצור עומק, והעליונה לבנה בוהקת
            let textColor = isTop ? '#FFFFFF' : '#111111';
            let strokeColor = isTop ? '#00E5FF' : '#000000';
            let strokeThick = isTop ? 10 : 25;

            let layerText = this.add.text(0, i * 4, 'THE HIDDEN\nPATH', {
            ...
            if (isTop) layerText.setShadow(0, 0, '#00E5FF', 30);
            this.titleContainer.add(layerText);
        }

        // מתחיל עצום ובלתי נראה
        this.titleContainer.setScale(25);
        this.titleContainer.setAlpha(0);

        // מופיע במהירות (Alpha)
        this.tweens.add({ targets: this.titleContainer, alpha: 1, duration: 400 });

        // זום-אאוט איטי ועוצמתי (נחיתה תלת מימדית)
        this.tweens.add({
            targets: this.titleContainer,
            scale: 1,
            duration: 2500,
            ease: 'Expo.easeOut', // מתחיל מהר מאוד ומאט לקראת הסוף
            onComplete: () => {
                // כשהוא נוחת - המסך רועד אימפקט של משקל
                this.cameras.main.shake(500, 0.015);
                
                // --- טיסה אל תוך הטקסט למעבר לסצנה הבאה ---
                this.time.delayedCall(800, () => {
                    // המצלמה ממקדת את עצמה וחותכת קדימה
                    this.cameras.main.pan(this.scale.width/2, this.scale.height/2 + 40, 900, 'Expo.easeIn');
                    this.cameras.main.zoomTo(60, 900, 'Expo.easeIn');
                    
                    // מסך מלבין כדי שהמעבר יהיה חלק לגמרי
...
class LobbyScene extends Phaser.Scene {
    ...
    create() {
        ...
        const noLives = window.PlayerData.triesLeft <= 0;

        if (isSolved) {
            this.createButton(this.scale.width/2, 850, '🔒 SOLVED', 0x9E9E9E, () => {});
        } else if (noLives) {
            this.createButton(this.scale.width/2, 850, '🔒 0 LIVES', 0x9E9E9E, () => {});
        } else {
...
class ShopScene extends Phaser.Scene {
    ...
    createShopCard(x, y, item) {
        ...
        let previewImg = null;
        if (this.textures.exists(`preview_${item.id}`)) {
            previewImg = this.add.image(0, -110, `preview_${item.id}`);
            let scale = Math.min(380 / previewImg.width, 260 / previewImg.height, 1); // לא מגדיל יותר מדי
        ...
        let cardElements = [bg, imgBox];
        if (previewImg) cardElements.push(previewImg);
        cardElements.push(nameTxt, btnBg, btnTxt, btnZone);
        card.add(cardElements);
...
class LockerScene extends Phaser.Scene {
    ...
    createLockerItem(x, y, id, equippedId) {
        ...
        // מציאת השם האמיתי מהמאגר במקום להשתמש ב-ID
        let itemObj = null;
        if (this.currentTab === 'skin') itemObj = window.OwnedAssets.skins.find(i => i.id === id);
        if (this.currentTab === 'pack') itemObj = window.OwnedAssets.packs.find(i => i.id === id);
        if (this.currentTab === 'bg') itemObj = window.OwnedAssets.bgs.find(i => i.id === id);
        let displayName = itemObj ? itemObj.name : id.toUpperCase();
        ...
        let previewImg = null;
        if (this.textures.exists(`preview_${id}`)) {
            previewImg = this.add.image(0, -50, `preview_${id}`);
            let scale = Math.min(380 / previewImg.width, 260 / previewImg.height, 1);
            previewImg.setScale(scale);
        }
        
        const nameTxt = this.add.text(0, 150, displayName, { fontFamily: this.comicFont, fontSize: '35px', fill: '#FFF', fontStyle: 'bold' }).setOrigin(0.5);
        const zone = this.add.zone(0, 0, 460, 440).setInteractive();        zone.on('pointerup', async (pointer) => {
            if (Math.abs(pointer.downY - pointer.upY) > 15) return;
        ...
        let cardElements = [bg, imgBox];
        if (previewImg) cardElements.push(previewImg);
        cardElements.push(nameTxt, zone);
        card.add(cardElements);
...
class GameScene extends Phaser.Scene {
    ...
    drawRealisticGrid() { 
            ...
            for (let row = -1; row < this.gridSize; row++) {
                for (let col = -1; col <= this.gridSize; col++) {
                    ...
                    if (isExternalWall) { topColor = this.currentBiome.wall; sideColor = this.currentBiome.wallDark; textureKey = this.textures.exists('custom_wall') ? 'custom_wall' : 'tex_wall'; }                    else if (tileType === 9) { topColor = '#FFD54F'; sideColor = '#FFB300'; textureKey = 'tex_exit'; } 
                        ...
                        // מחקנו מכאן את ה-continue כדי שהקוד ימשיך ויצייר את המסגרת!
                    } 
                    ...
                    else if (tileType === 3) { topColor = this.currentBiome.floor; sideColor = this.currentBiome.floorDark; textureKey = this.textures.exists('custom_floor') ? 'custom_floor' : 'tex_floor'; }
                    let baseRowDepth = (row + 2) * 30 + (col + 2) * 2;
                    let localGfx = this.add.graphics(); localGfx.setDepth(baseRowDepth);
                    ...
                    if (isWater) {
                        ...
                        // התיקון הענק לטראפים: מציירים קודם את הרצפה, ועליה מניחים את התמונה ללא שום חיתוך!
                        if (tileType === 3) {
                            ...
                            if (textureKey && this.textures.exists(textureKey)) {
                                ...
                                topImg = this.add.image(centerX, centerY, textureKey);
                                ...
                                topImg.setDisplaySize(tileW, tileH); // מתאים את הגודל למשבצת בדיוק
                            }
                        } else {
                            if (textureKey && this.textures.exists(textureKey)) {
                                let pts = [tl, tr, br, bl];
        ...
        if (this.playerContainer && this.playerContainer.active && this.bridgeVisualY !== undefined) {
            ...
            let playerDepth = (pRow + 2) * 30 + (this.playerLogic.x + 2) * 2 + 25; 
    ...
    updateBridgeVisuals() {
        ...
        // ציור חץ שמראה לאן הגשר עומד לזוז
        const bCX = (b_tl.x + b_tr.x + b_bl.x + b_br.x) / 4; 
        const bCY = (b_tl.y + b_tr.y + b_bl.y + b_br.y) / 4;
        let arrowSize = 10;
        let pDir = this.bridgeLogic.dir; 
        let isX = this.bridgeLogic.axis === 'x';
        
        this.bridgeObj.gfx.fillStyle(0xFFFFFF, 1);
        this.bridgeObj.gfx.lineStyle(2, 0x000000, 1);
        this.bridgeObj.gfx.beginPath();
        if (isX) {
            this.bridgeObj.gfx.moveTo(bCX + pDir * arrowSize, bCY);
            this.bridgeObj.gfx.lineTo(bCX - pDir * arrowSize, bCY - arrowSize);
            this.bridgeObj.gfx.lineTo(bCX - pDir * arrowSize, bCY + arrowSize);
        } else {
            this.bridgeObj.gfx.moveTo(bCX, bCY + pDir * arrowSize);
            this.bridgeObj.gfx.lineTo(bCX - arrowSize, bCY - pDir * arrowSize);
            this.bridgeObj.gfx.lineTo(bCX + arrowSize, bCY - pDir * arrowSize);
        }
        this.bridgeObj.gfx.closePath();
        this.bridgeObj.gfx.fillPath();
        this.bridgeObj.gfx.strokePath();
    }
    ...
    updateTrapsVisuals() {
        this.spikeTraps.forEach(trap => {
            ...
            if (this.trapObjects && this.trapObjects[key]) {
                ...
                // חישוב ארבעת הפינות של המשבצת בפרספקטיבה
                const tl = this.getPerspective(trap.x - 0.32, trap.y - 0.32, 2);
                const tr = this.getPerspective(trap.x + 0.32, trap.y - 0.32, 2);
                const bl = this.getPerspective(trap.x - 0.32, trap.y + 0.32, 2);
                const br = this.getPerspective(trap.x + 0.32, trap.y + 0.32, 2);

                // קביעת צבע האיקס: אדום כשהמלכודת דלוקה, אפור כשהיא כבויה
                const xColor = trap.active ? 0xFF3333 : 0x777777;
                
                // ציור קווי ה-איקס
                this.trapObjects[key].spikeGfx.lineStyle(8, xColor, 1);
                this.trapObjects[key].spikeGfx.beginPath();
                this.trapObjects[key].spikeGfx.moveTo(tl.x, tl.y);
                this.trapObjects[key].spikeGfx.lineTo(br.x, br.y);
                this.trapObjects[key].spikeGfx.moveTo(tr.x, tr.y);
                this.trapObjects[key].spikeGfx.lineTo(bl.x, bl.y);
    ...
    updatePlayerVisualPosition(duration = 0) {
        ...
        // הגנה שמוודאת שהשחקן באמת קיים לפני שמחליפים לו סקין כיוון
        if (this.playerSprite && this.playerSprite.active) {
            ...
            if (skinConfig) {
                // המרה חכמה: כיוון המשחק לכיוון של התמונות בארכיון 
                // [0] = למטה (אלינו), [1] = ימינה, [2] = למעלה (גב), [3] = שמאלה
                const dirMap = { 0: 1, 1: 0, 2: 3, 3: 2 };
                this.playerSprite.setTexture(`${skinConfig.id}_${dirMap[this.playerLogic.direction]}`);
            }
        } else if (this.eyeGraphics && this.eyeGraphics.active) {
        ...
        if (duration > 0) {
            this.tweens.add({
                ...
                onUpdate: (tween) => {
                    ...
                    if (this.playerGraphic && this.playerGraphic.active) { this.playerGraphic.y = -jumpHeight; this.eyeGraphics.y = -jumpHeight; }
                    if (this.playerSprite && this.playerSprite.active) { this.playerSprite.y = -50 - jumpHeight; }
                },
                onComplete: () => {
                    if (this.playerGraphic && this.playerGraphic.active) { this.playerGraphic.y = 0; this.eyeGraphics.y = 0; }
                    if (this.playerSprite && this.playerSprite.active) { this.playerSprite.y = -50; }
    ...
    createPlayer() {
        ...
        // התיקון הקריטי לתקיעות: איפוס מוחלט של זיכרון השחקן הישן!
        ...
        if (skinConfig.isDefault) {
            ...
            this.playerGraphic.fillRoundedRect(-30, -70, 60, 60, 12); this.playerGraphic.strokeRoundedRect(-30, -70, 60, 60, 12);
            
            this.eyeGraphics = this.add.graphics();
            this.eyeGraphics.fillStyle(0xFFFFFF, 1); this.eyeGraphics.lineStyle(2, 0x000000, 1);
            this.eyeGraphics.fillCircle(-12, -50, 10); this.eyeGraphics.strokeCircle(-12, -50, 10);
            this.eyeGraphics.fillCircle(12, -50, 10); this.eyeGraphics.strokeCircle(12, -50, 10);
            this.eyeGraphics.fillStyle(0x000000, 1);
            this.eyeGraphics.fillCircle(-12, -50, 4); this.eyeGraphics.fillCircle(12, -50, 4);
            
            this.playerContainer.add([this.playerGraphic, this.eyeGraphics]);
        } else {
            this.playerSprite = this.add.image(0, -50, `${skinConfig.id}_3`); 
    ...
    setupBoardInteraction() {
        this.input.on('pointerdown', (pointer) => {
            ...
            if (clickedCol !== -1) {
                ...
                else if (isMovingBridge || tileType === 4) { 
                    title = isMovingBridge ? 'Moving Bridge' : 'Static Bridge'; 
                    subtitle = isMovingBridge ? `Move In: ${this.bridgeLogic.pauseTimer}` : 'Safe'; 
                }
                else if (tileType === 2) { title = 'Deep Water'; subtitle = 'Danger: Sinks Player'; }
                else if (tileType === 3) { 
                    title = 'Spike Trap'; 
                    subtitle = `Move In: ${this.trapTimer}`; 
                }
    ...
    calculateCurrentActionCost() {
        let cost = 0; 
        this.actionQueue.forEach(a => { 
            if(a.type !== 'LOOP_END' && a.type !== 'LOOP_START') cost++; 
            if(a.type === 'LOOP_START') cost++; 
        });
        return cost;
    }

    renderVisualQueue(animateMovement = false) {
        const rowsCount = Math.min(4, Math.ceil(this.actionQueue.length / this.blocksPerRow)) || 1;
    ...
    triggerDeath(reason) {
        this.cumulativeActions = 0; // מוות בקיר/מים/מלכודת מאפס את הצבירה!
    ...
    showGameOverScreen() {
        ...
        fetch('/api/announce', {
            ...
            body: JSON.stringify({
                channelId: window.PlayerData.channelId, isWin: false, score: 0, tries: this.maxTries, crystals: this.collectedCrystals, moves: 0, 
                biome: this.currentBiome, 
                themeColor: '#' + (window.OwnedAssets.bgs.find(b => b.id === window.PlayerData.currentBG) || {uiMain: 0xFF5252}).uiMain.toString(16).padStart(6, '0'),
                skin: window.OwnedAssets.skins.find(s => s.id === window.PlayerData.currentSkin),
                bgTheme: window.OwnedAssets.bgs.find(b => b.id === window.PlayerData.currentBG),
                pack: window.OwnedAssets.packs.find(p => p.id === window.PlayerData.currentPack)
            })
        }).catch(e => console.log(e));
        
        const panel = this.add.graphics(); 
        panel.fillStyle(0x000000, 0.85); panel.fillRect(0, 0, this.scale.width, this.scale.height); panel.setDepth(10000);
        
        const box = this.add.graphics();
        box.fillStyle(0x2E3136, 1); box.lineStyle(10, 0xFF5252, 1);
        box.fillRoundedRect(this.scale.width/2 - 450, 300, 900, 700, 40); box.strokeRoundedRect(this.scale.width/2 - 450, 300, 900, 700, 40); box.setDepth(10001);
        
        this.add.text(this.scale.width/2, 450, 'GAME OVER', { fontFamily: this.comicFont, fontSize: '90px', fill: '#FF5252', fontStyle: 'bold', stroke: '#000', strokeThickness: 12, align: 'center' }).setOrigin(0.5).setDepth(10001);
        this.add.text(this.scale.width/2, 650, 'NO LIVES REMAINING TODAY', { fontFamily: this.comicFont, fontSize: '40px', fill: '#FFF' }).setOrigin(0.5).setDepth(10001);
        this.add.text(this.scale.width/2, 750, 'Wait for the daily reset!', { fontFamily: this.comicFont, fontSize: '30px', fill: '#AAAAAA' }).setOrigin(0.5).setDepth(10001);
        
        const btnBg = this.add.graphics(); btnBg.fillStyle(0xFF5252, 1); btnBg.lineStyle(6, 0x000, 1);
        btnBg.fillRoundedRect(this.scale.width/2 - 300, 850, 600, 100, 25); btnBg.strokeRoundedRect(this.scale.width/2 - 300, 850, 600, 100, 25); btnBg.setDepth(10001);
        this.add.text(this.scale.width/2, 900, 'RETURN TO LOBBY', { fontFamily: this.comicFont, fontSize: '40px', fill: '#FFF', fontStyle: 'bold', stroke: '#000', strokeThickness: 6 }).setOrigin(0.5).setDepth(10002);
        
        const btnZone = this.add.zone(this.scale.width/2, 900, 600, 100).setInteractive().setDepth(10003);
    ...
    showWinScreen() {
        ...
        let finalActionCost = this.cumulativeActions + actionCost; // מחבר את כל המהלכים!

        fetch('/api/win', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': window.PlayerData.accessToken },
            body: JSON.stringify({ actionCost: finalActionCost }) // שולח את המספר הסופי לשרת
            ...
            fetch('/api/announce', {
                ...
                body: JSON.stringify({
                    ...
                    crystals: this.collectedCrystals, moves: actionCost, biome: this.currentBiome, 
                    themeColor: '#' + (window.OwnedAssets.bgs.find(b => b.id === window.PlayerData.currentBG) || {uiMain: 0xFFD54F}).uiMain.toString(16).padStart(6, '0'),
                    skin: window.OwnedAssets.skins.find(s => s.id === window.PlayerData.currentSkin),
                    bgTheme: window.OwnedAssets.bgs.find(b => b.id === window.PlayerData.currentBG),
                    pack: window.OwnedAssets.packs.find(p => p.id === window.PlayerData.currentPack)
                })
            ...
            box.fillRoundedRect(this.scale.width/2 - 450, 200, 900, 1000, 40); box.strokeRoundedRect(this.scale.width/2 - 450, 200, 900, 1000, 40); box.setDepth(10001);
            
            this.add.text(this.scale.width/2, 350, 'MISSION\nACCOMPLISHED', { fontFamily: this.comicFont, fontSize: '80px', fill: '#FFD54F', fontStyle: 'bold', stroke: '#000000', strokeThickness: 12, align: 'center' }).setOrigin(0.5).setDepth(10001);
            
            this.add.text(this.scale.width/2, 600, `💎 Crystals: ${this.collectedCrystals}/3`, { fontFamily: this.comicFont, fontSize: '45px', fill: '#00E5FF', fontStyle: 'bold' }).setOrigin(0.5).setDepth(10001);
            this.add.text(this.scale.width/2, 680, `🧩 Blocks Used: ${finalActionCost} (Par: ${data.parScore})`, { fontFamily: this.comicFont, fontSize: '45px', fill: '#FFFFFF' }).setOrigin(0.5).setDepth(10001);
            
            this.add.text(this.scale.width/2, 850, `SCORE: ${data.totalScore}`, { fontFamily: this.comicFont, fontSize: '85px', fill: '#4CAF50', fontStyle: 'bold', stroke: '#000000', strokeThickness: 10 }).setOrigin(0.5).setDepth(10001);
            this.add.text(this.scale.width/2, 980, `+ 🪙 ${data.earnedCoins} COINS!`, { fontFamily: this.comicFont, fontSize: '60px', fill: '#FFC107', fontStyle: 'bold', stroke: '#000', strokeThickness: 10 }).setOrigin(0.5).setDepth(10001);

            const btnBg = this.add.graphics(); btnBg.fillStyle(0x4CAF50, 1); btnBg.lineStyle(6, 0x000000, 1);
            btnBg.fillRoundedRect(this.scale.width/2 - 300, 1080, 600, 100, 25); btnBg.strokeRoundedRect(this.scale.width/2 - 300, 1080, 600, 100, 25); btnBg.setDepth(10001);
            this.add.text(this.scale.width/2, 1130, 'COLLECT & RETURN', { fontFamily: this.comicFont, fontSize: '40px', fill: '#FFF', fontStyle: 'bold', stroke: '#000', strokeThickness: 6 }).setOrigin(0.5).setDepth(10002);
            
            const btnZone = this.add.zone(this.scale.width/2, 1130, 600, 100).setInteractive().setDepth(10003);
    ...
    updateDynamicElements() {
        if (this.bridgeLogic) {
            let oldBx = this.bridgeLogic.x;
            let oldBy = this.bridgeLogic.y;

            if (this.bridgeLogic.pauseTimer > 0) this.bridgeLogic.pauseTimer--;
            else {
                if (this.bridgeLogic.axis === 'x') this.bridgeLogic.x += this.bridgeLogic.dir;
                else this.bridgeLogic.y += this.bridgeLogic.dir;
                
                let currPos = this.bridgeLogic.axis === 'x' ? this.bridgeLogic.x : this.bridgeLogic.y;
                if (currPos >= this.bridgeLogic.max || currPos <= this.bridgeLogic.min) this.bridgeLogic.dir *= -1;
                this.bridgeLogic.pauseTimer = 1;
                this.bridgeVisualX = this.bridgeLogic.x;
                this.bridgeVisualY = this.bridgeLogic.y;

                // 🌟 בדיקה: אם השחקן עומד על הגשר הנוכחי, נסיע אותו יחד עם תנועת הגשר!
                if (this.playerLogic.x === oldBx && this.playerLogic.y === oldBy) {
                    if (this.bridgeLogic.axis === 'x') {
                        this.playerLogic.x += (this.bridgeLogic.x - oldBx);
                    } else {
                        this.playerLogic.y += (this.bridgeLogic.y - oldBy);
                    }
                    
                    // הזזה ויזואלית חלקה של השחקן יחד עם פאנל הגשר
                    const newPos = this.getPerspective(this.playerLogic.x, this.playerLogic.y, 5);
                    this.tweens.add({
        ...
        if (this.trapTimer > 0) {
            ...
            this.spikeTraps.forEach(t => t.active = !t.active);
            this.trapTimer = 1;
        }
        this.updateTrapsVisuals();
    }
    ...
    executeNextAction() {
        ...
        if (this.currentStep >= this.actionQueue.length) { 
            // תיקון בטיחות: מוודא שהשחקן בתוך המפה לפני שהוא בודק ניצחון
            let onExit = false;
            if (this.mapData[this.playerLogic.y] && this.mapData[this.playerLogic.y][this.playerLogic.x] === 9) {
                onExit = true;
            }
            ...
            if (!onExit) {
                ...
                this.cumulativeActions += actionCost; // שומרים את המהלכים של הסיבוב הזה
